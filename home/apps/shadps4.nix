...
in {
  ...
  xdg.desktopEntries.bloodborne = lib.mkIf pkgs.stdenv.isLinux {
    ...
    exec = "env QT_QPA_PLATFORM=xcb MANGOHUD=1 taskset -c 0-15 gamemoderun gamescope -W 2560 -H 1440 -r 120 -f --adaptive-sync --hdr-enabled --force-grab-cursor -- ${pkgs.bblauncher}/bin/BB_Launcher -n";
